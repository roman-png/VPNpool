...
var DICT = {
	ru: {
		...
		'Smart bypass': 'Умный обход',
		'on (direct DPI bypass)': 'вкл (прямой обход DPI)',
		'off': 'выкл',
		'self-learned: %s': 'самообучено: %s',
		'direct: %s': 'напрямую: %s',
		'anti-throttle': 'антитроттлинг',
		'Lite mode: zapret direct bypass ONLY — no proxy/sing-box (for tiny 16 MB routers)': 'Lite-режим: ТОЛЬКО прямой обход через zapret — без прокси/sing-box (для крошечных 16 МБ роутеров)',
		'Lite mode turns OFF the proxy/sing-box and runs only zapret direct bypass. Continue?': 'Lite-режим ВЫКЛЮЧАЕТ прокси/sing-box и оставляет только прямой обход zapret. Продолжить?',
		'LITE': 'LITE',
		'Tune desync strategy': 'Подобрать стратегию desync',
		'finds a working DPI-bypass strategy for your ISP (blockcheck) — run once after install': 'находит рабочую стратегию обхода DPI для вашего провайдера (blockcheck) — запустите один раз после установки',
		'Tuning desync strategy via blockcheck… this can take a few minutes.': 'Подбираю стратегию desync через blockcheck… это может занять несколько минут.',
		'Desync strategy tuned — %s: %s': 'Стратегия desync подобрана — %s: %s',
		'verified working': 'проверено, работает',
		'applied (not confirmed)': 'применено (не подтверждено)',
		'Strategy tuning failed: %s': 'Подбор стратегии не удался: %s',
		'zapret installed (%s). Auto-tuning desync strategy in the background…': 'zapret установлен (%s). Автоподбор стратегии desync в фоне…',
		...
		'AmneziaWG: paste an AmneziaWG .conf or an AmneziaVPN vpn:// link here (not in the single field above) — it joins the same pool. The single field above is for vless:// / vmess:// / trojan:// / ss:// links only.':
			'AmneziaWG: вставляйте .conf или ссылку vpn:// сюда (не в одиночное поле выше) — узел войдёт в тот же пул. Одиночное поле выше — только для ссылок vless:// / vmess:// / trojan:// / ss://.',
		'AmneziaWG: paste an AmneziaWG .conf or an AmneziaVPN vpn:// link here too — it joins the same pool.':
			'AmneziaWG: сюда также можно вставить .conf или ссылку vpn:// — узел войдёт в тот же пул.',
