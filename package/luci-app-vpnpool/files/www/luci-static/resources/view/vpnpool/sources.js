...
return view.extend({
	...
	//     vpn:// link, or load a file. vpn:// is decoded server-side (rpcd awgdecode.uc),
	//     so the browser just sends the raw text — no client-side zlib dependency. --------
	handleImportNodes: function(ta) {
		...
		return callImportNodes(txt).then(L.bind(function(r) {
			if (r && r.ok) {
				ta.value = '';
				this.notify(r.awg ? _('AmneziaWG node imported.') : _('Imported %d new node(s) (manual list: %d).').replace('%d', (r.added != null ? r.added : 0)).replace('%d', (r.total != null ? r.total : 0)));
				this.reload();
			} else {
