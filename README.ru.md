...
- 🩺 **Watchdog самовосстановления туннеля** — супервизор проверяет туннель насквозь
  (а не только pid sing‑box); если туннель мёртв ~2 минуты при живом WAN — sing‑box
  перезапускается автоматически, устаревший кэш urltest сбрасывается.
...
- 🤖 **Двусторонний Telegram‑бот** — `/status /nodes /switch /speedtest /quota /saved
  /clients /on /off /refresh`, доступ только с вашего chat id, через туннель.
- 📊 **Остаток трафика подписки** — парсит заголовок `subscription‑userinfo` и
  показывает использовано/всего ГБ с прогресс‑баром и сроком действия.
- 💾 **Сохранённые узлы** — отметьте узел звездой (⭐), чтобы держать его в отдельном
  постоянном **архиве**, переживающем окончание подписки. Сохранённые узлы, которых
  уже нет в активной подписке, показываются отдельным списком **«Сохранённые
  (неактивные)»** на дашборде; одной кнопкой **добавляйте их в активный пул** (или
  убирайте). Опциональный **авто‑снимок** сам сохраняет ограниченный резервный набор
  доступных сейчас узлов (ручные ⭐ никогда не вытесняются).
- 🔎 **Поиск/фильтр/сортировка узлов**, ⚡ **тест реальной скорости узла** (с проверкой
  свободной памяти на 16 МБ‑роутерах), 📈 **трафик по узлам и устройствам** (с именами DHCP).
- ⏰ **Планировщик** — вкл/выкл VPN и обновление подписки по расписанию (cron).
- 🔗 **Шеринг/экспорт узлов** — ссылка + **офлайн‑QR** у каждого узла (перенос на
  телефон) и экспорт сохранённых/ручных/всех узлов как base64‑подписки. QR рисуется
  **на клиенте** — секреты узлов не покидают роутер.
- 🧩 **Несколько подписок** — дополнительные полные подписки подмешиваются в пул.
- 🧠 **Адаптивная маршрутизация** — сама находит домены, **заблокированные напрямую**
  (RST/таймаут), и направляет в VPN именно их — список ведёт себя сам под ваш ISP.
  Плюс кнопка «этот сайт не открывается».
- 🎬 **Unlock-тест по узлам** — проверка, что открывает каждый узел (YouTube /
  ChatGPT / Netflix / Instagram / Telegram / Google), с бейджами прямо в дашборде.
- 🛡️ **Анти-DPI** — один тумблер для фрагментации TLS-хендшейка (sing-box
  `tls_fragment`) против SNI-DPI.
- 🛡️ **Kill‑switch (IPv4)** и **защита от DNS‑утечек** (опционально).
...
| **Unlock‑тест по узлам** | ✅ | — | — | — |
| **Анти‑DPI (TLS‑фрагментация)** | ✅ тумблер | — | — | — |
| **Остаток трафика подписки** | ✅ исп./всего + бар | — | — | — |
| **Сохранённые узлы (после окончания)** | ✅ | — | — | — |
| **Тест скорости узла** | ✅ throughput | — | — | — |
| **Статистика по узлам/устройствам** | ✅ live | — | частично | частично |
