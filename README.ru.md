...
> Фид без подписи. Если opkg ругнётся на отсутствие подписи — добавьте
> `option check_signature 0` в начало `/etc/opkg.conf` либо `--no-check` к командам
> `opkg`. Не хотите трогать конфиг opkg — используйте вариант A/B.
