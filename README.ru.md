...
- 📡 **Автообновляемая подписка** — вставьте URL подписки (список base64 **или**
  ...
  конфигов из репозитория). **Перебор клиентских User‑Agent**: пробует несколько UA, но
  **останавливается на первом, который вернул узлы** (к следующему UA переходит, только
  если ответ пуст) — работает, если провайдер отдаёт определённому клиенту, и не долбит
  панель всеми UA подряд.
...
- 🩺 **Watchdog самовосстановления туннеля** — супервизор проверяет туннель насквозь
  (а не только pid sing‑box); если туннель держится мёртвым при живом WAN (после
  grace‑прогрева, подтверждённо несколькими пробами) — sing‑box перезапускается
  автоматически, устаревший кэш urltest сбрасывается.
- 🧹 **Отсев мёртвых узлов** — недоступные узлы проверяются по TCP при сборке и не
  попадают в пул авто‑пинга (urltest), чтобы подписка с кучей мёртвых серверов не
  копила висящие сокеты проб и не глушила пинг живых узлов (для ручного выбора они
  остаются доступны).
- 🌐 **DNS со стратегией IPv4** — собственные health‑пробы sing‑box резолвятся сначала
  по IPv4 (`dns_strategy`, по умолчанию `prefer_ipv4`), чтобы роутер, получающий
  AAAA‑записи без IPv6‑транзита через узлы, не показывал ложный «0 пинга» у рабочих узлов.
- 📌 **Приоритетный узел с возвратом** — закрепите любимый узел **прямо на Дашборде**
  (кнопка 📌 в строке узла): он используется, пока доступен; при падении управление
  передаётся авто‑режиму, а после восстановления **возвращается обратно** (с защитой
  от дёрганья поверх tolerance urltest). Применяется на лету — без перезапуска туннеля.
  На Дашборде режим **АВТО** остаётся подсвеченным, пока узел закреплён — мягкий пин
  лишь смещает авто‑режим, это не жёсткий ручной выбор.
...
- 👥 **Маршрутизация по устройствам** — вся сеть, **исключить** конкретные устройства
  (идут мимо VPN) или разрешить **только** определённые устройства. Устройства можно
  выбирать **по имени из списка DHCP** (сопоставление по **MAC**, поэтому профиль
  переживает смену IP по DHCP); статические/неизвестные хосты по‑прежнему можно
  добавить как IPv4 вручную.
- 🧩 **Протоколы** — VLESS (Reality + `xtls‑rprx‑vision`), VMess, Trojan, Shadowsocks,
  а также готовые JSON‑конфиги sing‑box. **AmneziaWG** (обфусцированный WireGuard, проходит
  RU‑DPI на мобильных, где TLS/Reality сбрасывается) — полноценный член пула: импортируйте
  `.conf` или ссылку AmneziaVPN `vpn://` на вкладке **Источники** (vpn:// декодируется прямо
  на роутере, приложение Amnezia не нужно). Узел входит в тот же пул авто‑пинга/failover и
  проверяется на сервисы как остальные. AmneziaWG требует форк sing‑box с AWG — ставится
  флагом `VPNPOOL_AWG=1` (см. Установку); пребилд для `aarch64`/`mipsel`.
...
- ⚡ **Умный обход (прямой обход DPI)** — если установлен отдельный **zapret**, один
  тумблер переводит его в режим **autohostlist**: nfqws **сам учится** на заблокированных
  DPI сайтах и пробивает их **напрямую** (без прокси, поэтому переживает троттлинг
  прокси). vpnpool лишь управляет установленным zapret (своя nft-таблица + fwmark
  `0x40000000`, уживается с туннелем) и показывает его как обнаруженный, со счётчиком
  самообученных доменов. Прокси остаётся для гео-блоков, которые desync не чинит.
  Кнопка **«Установить zapret»** в один клик скачивает официальный пакет под
  архитектуру вашего роутера и ставит его (вместе с kmod NFQUEUE). **Автоподбор**
  находит рабочую desync‑стратегию для *вашего* провайдера через `blockcheck` и
  прописывает её в zapret (запускается автоматически сразу после установки).
  **3‑сторонний классификатор** решает per‑домен: *desync‑напрямую* или *прокси*
  (гео‑блок уходит в прокси, DPI‑блок остаётся напрямую). Опциональный
  **антитроттлинг** сам включает прямой обход, когда прокси душат в ноль. А
  **Lite‑режим** запускает только zapret **без sing‑box** для крошечных 16 МБ
  роутеров (nfqws ~1.5 МБ и живёт во флеше).
...
> **AmneziaWG (`VPNPOOL_AWG=1`):** добавьте эту переменную к любому one‑liner, чтобы
> использовать форк sing‑box с AmneziaWG вместо стокового (сток не умеет AmneziaWG).
> Пребилд для `aarch64` и `mipsel`; на флеш он заменяет `/usr/bin/sing‑box` и ставит
> `opkg hold`, на малой флеш WAN‑хук тянет форк в ОЗУ. Затем импортируйте `.conf` /
> ссылку `vpn://` на вкладке **Источники**. ⚠ podkop (если установлен) использует тот же
> бинарник и тоже поедет на форке.
