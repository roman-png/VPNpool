...
Один раз добавьте наш [opkg‑фид на GitHub Pages](https://roman-png.github.io/VPNpool),
дальше ставьте/обновляйте как обычный пакет:
...
> Фид без подписи, поэтому проверка подписи opkg (включена по умолчанию) отклонит
> его и удалит список пакетов. Отключите проверку, **закомментировав** (или удалив)
> строку `option check_signature` в `/etc/opkg.conf` — учтите, что значение `0`
> **не** помогает, строку нужно именно убрать/закомментировать — затем повторите
> `opkg update`. Если не хотите менять политику подписи opkg — используйте вариант A/B.
